...
const parseExtensionInfo = async (url) => {
  try {
    ...
    return { extensionName, publisher, version };
  } catch (error) {
      ...
      if (totalSize) {
        ...
        if (progress >= lastProgress + 10 || progress === 100) {
          process.stdout.write(
            `\r下载进度: ${progress}% (${formatBytes(downloadedSize)}/${formatBytes(totalSize)})`,
          );
  ...
  .action(async (publisher, extension, version, options) => {
    try {
      ...
      if (!fs.existsSync(outputDir)) {
        fs.mkdirSync(outputDir, { mode: 0o755, recursive: true });
      }
  ...
  .action(async (file, options) => {
    try {
      ...
      for (const plugin of plugins) {
        try {
          const { extension, publisher, version } = plugin;
          const downloadUrl = generateDownloadUrl(
            publisher,
            extension,
            version,
